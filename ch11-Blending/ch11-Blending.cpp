...
class BlendApp : public D3D11App
{
public:
...
bool BlendApp::InitStatus()
{
	...
	hr = pDevice->CreateRasterizerState(&cmdesc, &pCCW);

	cmdesc.FrontCounterClockwise = false;
	hr = pDevice->CreateRasterizerState(&cmdesc, &pCW);

	return true;
}

bool BlendApp::InitBuffer()
{
	Vertex VertexData[] =
	{
		// Front Face
...
bool BlendApp::InitShader()
{

	hr = D3DX11CompileFromFile(L"blend.fx", 0, 0, "VS", "vs_4_0", 0, 0, 0, &VS_Buffer, 0, 0);
	hr = D3DX11CompileFromFile(L"blend.fx", 0, 0, "PS", "ps_4_0", 0, 0, 0, &PS_Buffer, 0, 0);
...
void BlendApp::RenderScene()
{
	...
	pDeviceContext->RSSetState(pCCW);
	pDeviceContext->DrawIndexed( 36, 0, 0 );
	pDeviceContext->RSSetState(pCW);
	pDeviceContext->DrawIndexed( 36, 0, 0 );
