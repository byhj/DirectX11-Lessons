...
bool TextureApp::InitStatus()
{
	...
	// Create the texture
	pD3D11Device->CreateTexture2D(&textureDesc, NULL, &renderTargetTextureMap);
	...
	pD3D11Device->CreateRenderTargetView(renderTargetTextureMap, &renderTargetViewDesc, &renderTargetViewMap);
	...
	pD3D11Device->CreateShaderResourceView(renderTargetTextureMap, &shaderResourceViewDesc, &shaderResourceViewMap);
...
void TextureApp::RenderScene()
{
	...
	pD3D11DeviceContext->OMSetRenderTargets( 1, &renderTargetViewMap, pDepthStencilView );

	// Now clear the render target
	pD3D11DeviceContext->ClearRenderTargetView(renderTargetViewMap, bgColor);
	...
	pD3D11DeviceContext->UpdateSubresource( cbPerObjectBuffer, 0, NULL, &cbPerObj, 0, 0 );
	pD3D11DeviceContext->VSSetConstantBuffers( 0, 1, &cbPerObjectBuffer );
	pD3D11DeviceContext->PSSetShaderResources( 0, 1, &shaderResourceViewMap );	// Draw the map to the square
	pD3D11DeviceContext->PSSetSamplers( 0, 1, &CubesTexSamplerState );
