...
/************************************New Stuff****************************************************/
void TextureApp::SaveTreeBillboardTexture()
{
	...
	// We will have the width of the texture be 512 pixels, and the height of the texture depends on the height:width ratio of the bounding box (y/x)

	float treeWidth = treeAABB[1].x - treeAABB[0].x;	// Get length of AABB along x axis
	float treeHeight = treeAABB[1].y - treeAABB[0].y;	// Get lenght of AABB along y axis
	...
	// Create the texture
	pD3D11Device->CreateTexture2D(&textureDesc, NULL, &billRenderTargetTexture);
	...
	pD3D11Device->CreateRenderTargetView(billRenderTargetTexture, &renderTargetViewDesc, &billRenderTargetView);
	...
	XMVECTOR billCamPosition = XMVectorSet(treeAABB[2].x, treeAABB[2].y , treeAABB[0].z - 20.0f, 0.0f);
	XMVECTOR billCamTarget = XMVectorSet(treeAABB[2].x, treeAABB[2].y, treeAABB[0].z, 0.0f);
	...
	pD3D11DeviceContext->ClearRenderTargetView(billRenderTargetView, bgColor);	
	...
	pD3D11DeviceContext->OMSetRenderTargets( 1, &billRenderTargetView, pDepthStencilView );
	...
	/******************* Draw Whatever it is we want to save to a file ***********************/
	...
	WVP = treeWorld * billView * billProjection;
	...
	pD3D11DeviceContext->DrawIndexedInstanced( 6, numLeavesPerTree, 0, 0, 0 );	// Only Draw a single Tree's worth of leaves
	...
	for(int i = 0; i < treeSubsets; ++i)
	{
		...
		if(!material[meshSubsetTexture[i]].transparent)
			pD3D11DeviceContext->DrawIndexedInstanced( indexDrawAmount, 1, indexStart, 0, 0 );	// Only Draw a single Tree
	}

	D3DX11SaveTextureToFile(pD3D11DeviceContext, billRenderTargetTexture, D3DX11_IFF_PNG, L"treeBillTexture.png");

	billRenderTargetTexture->Release();
	billRenderTargetView->Release();
}

void TextureApp::getBillboards(std::vector<InstanceData> &instanceDataPos)
{
	...
	pD3D11DeviceContext->UpdateSubresource( treeBillboardInstanceBuff, 0, NULL, &treeBillboardInstanceData[0], 0, 0 );
}
/*************************************************************************************************/
...
std::vector<XMFLOAT3> TextureApp::CreateAABB(std::vector<XMFLOAT3> &vertPosArray)
{
	...
	// Our AABB [0] is the min vertex, [1] is the max, and [2] is the objects true center
...
bool TextureApp::InitBuffer()
{
	...
	// We will store the billboards width and height in the "instance cb" or scene cb
	...
	hr = D3DX11CompileFromFile(L"skybox.fx", 0, 0, "GS_Billboard", "gs_4_0", 0, 0, 0, &GS_Buffer, 0, 0);
	...
	hr = pD3D11Device->CreateGeometryShader(GS_Buffer->GetBufferPointer(), GS_Buffer->GetBufferSize(), NULL, &GS);
...
bool TextureApp::InitStatus()
{
	...
	hr = pD3D11Device->CreateBuffer( &instBuffDesc, 0, &treeBillboardInstanceBuff);
	...
	pD3D11Device->CreateBuffer(&billboardBufferDesc, &ginitData, &treeBillboardVertBuff);

	// Create billboard texture
	// Call the function which saves our tree billboard texture to a file (uncomment if you want to save the billboard texture)
	//SaveTreeBillboardTexture();	

	// The function above changes the viewport, so we have to change the viewport back to the one we defined above
	pD3D11DeviceContext->RSSetViewports(1, &viewport);

	// Now we load in the tree's billboard texture
	// In the actual applications, you will just call the above function during developement. After you
	// have the billboards texture on file, there is no need to call this function again every time the program
	// starts up
	hr = D3DX11CreateShaderResourceViewFromFile(pD3D11Device, L"treeBillTexture.png",
		NULL, NULL, &treeBillboardSRV, NULL );
	/*************************************************************************************************/
...
void TextureApp::RenderText(std::wstring text, int inInt)
{
	...
	printString << text << inInt << L"\n"
		<< L"Trees in sight: " << numTreesToDraw + numBillboardTreesToDraw;
	/*************************************************************************************************/
...
void TextureApp::RenderScene()
{
	...
	/************************************New Stuff****************************************************/	
	// Use same blending for our d2d stuff	
	pD3D11DeviceContext->OMSetBlendState(d2dTransparency, NULL, 0xffffffff);

	// Set topology to points
	pD3D11DeviceContext->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_POINTLIST );

	// Here we set the shaders, the vertex shader is the default one
	pD3D11DeviceContext->VSSetShader(pVS, 0, 0);
	// The geometry shader is the only one defined in the effects file at the time
	pD3D11DeviceContext->GSSetShader(GS, 0, 0);
	// The pixel shader is the one we used for d2d, since all we need to do is render at texture to the billboard
	pD3D11DeviceContext->PSSetShader(pD2D_PS, 0, 0);
	...
	//Set the models vertex and isntance buffer using the arrays created above
	pD3D11DeviceContext->IASetVertexBuffers( 0, 2, vertBillInstBuffers, strides, offsets );
	...
	pD3D11DeviceContext->VSSetConstantBuffers( 0, 2, vsBillConstBuffers );
	// Set Geometry shaders (we have 3 cb's to set, so we can use an array of buffers)
	ID3D11Buffer* gsBillConstBuffers[3] = {cbPerFrameBuffer, cbPerObjectBuffer, cbPerInstanceBuffer};
	pD3D11DeviceContext->GSSetConstantBuffers(0, 3, gsBillConstBuffers);	
	pD3D11DeviceContext->PSSetConstantBuffers( 1, 1, &cbPerObjectBuffer );
	pD3D11DeviceContext->PSSetShaderResources( 0, 1, &treeBillboardSRV );
	pD3D11DeviceContext->PSSetSamplers( 0, 1, &CubesTexSamplerState );

	pD3D11DeviceContext->RSSetState(RSCullNone);
	pD3D11DeviceContext->DrawInstanced( 1, numBillboardTreesToDraw, 0, 0 );

	// set gs and ps back to defaults
	pD3D11DeviceContext->GSSetShader(NULL, 0, 0);
	pD3D11DeviceContext->PSSetShader(pPS, 0, 0);
